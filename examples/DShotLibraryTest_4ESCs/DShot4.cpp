...
// Denote which pins is attached to dShot
...
// Mode: 600/300/150
static enum DShot4::Mode dShotMode = DShot4::Mode::DSHOT600;
...
DShot4::DShot4(const enum Mode mode) { dShotMode = mode; }

void DShot4::attach(uint8_t pin) {
  this->_packet = 0;
...
uint16_t DShot4::setThrottle(uint8_t pin, uint16_t throttle,
  ...
  for (byte i = 0; i < 16; i++) {
    if (this->_packet & mask)
      dShotBits[i] |= bitMaskPin;  // this->_pinMask;
    else
      dShotBits[i] &= bitMaskPin;  //~(this->_pinMask);
    mask >>= 1;
  }
